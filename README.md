...
2. **组合模拟与回测**：创建投资组合，支持按历史财务快照回溯调测选股参数，并跟踪组合收益。
...
| 数据类型 | 来源 | 说明 |
|----------|------|------|
| 中证1000成分股 | Excel / CSV 文件导入 | 指数每年约调整一次，手动维护即可 |
...
过滤条件：
  1. PE > 0（排除亏损、无效 PE）
  2. ROE 有效
  3. 股息率 >= 用户设定阈值（默认 2%，可配置）

排序：
  score = ROE / PE

输出：
  取 score 排名前 N 只（如前 20 只）
```
...
| 股息率（%） | 行情或分红接口 | 需统一口径（建议 TTM） |
| 收盘价 | 行情接口 | 组合收益计算 |

### 采集注意事项

- 中证1000 约 1000 只股票，全量采集需 **限速**（建议每只间隔 200–500ms）
- 接口非官方公开文档，可能变更；统一封装在 `EastMoneyDataSource`，便于维护
- 每次采集结果写入 `financial_snapshot`，支撑历史回溯
